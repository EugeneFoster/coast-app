...
export default async function ProjectsPage({
  ...
  return (
    <div className="px-3 pb-8 pt-3 md:px-4 md:pt-4 xl:px-6 xl:pb-10 xl:pt-6">
      <section>
        <div className="hidden items-end justify-between gap-4 xl:flex">
          <div>
          ...
          <div className="flex items-center gap-2">
            ...
            >
              <DesignIcon name="filter" size={16} />
              Filters
            </Link>
            {admin && (
              <Link
          ...
          view === "table" ? (
                      ...
                      >
                        <td className="px-4 py-3 font-medium text-ink">
                          <div className="flex items-center gap-3">
                            <div className="h-[34px] w-[52px] shrink-0">
                              <StructureThumbnail projectId={project.id} name={project.name} structureType={project.structure_type} compact />
                            </div>
                            <div>
                              <Link href={`/projects/${project.id}`}>{project.name}</Link>
                              <p className="mt-0.5 font-mono text-[11px] text-graph">PRJ-{project.id.slice(0, 4).toUpperCase()}</p>
                            </div>
                          </div>
                        </td>
            ...
            <div className="grid gap-3 md:grid-cols-2 md:gap-4 xl:grid-cols-3">
              {projects.map((project, index) => (
                <ProjectCard key={project.id} project={project} canEdit={admin} featured={index === 0} />
              ))}
            </div>
          )
        ) : (
